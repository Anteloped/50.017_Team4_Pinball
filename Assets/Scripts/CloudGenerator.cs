...
public class CloudGenerator : MonoBehaviour
{
    ...
    {
        for (float x = 0; x < width; x += UnityEngine.Random.Range(stepMin, stepMax))
        {
            for (float z = 0; z < depth; z += UnityEngine.Random.Range(stepMin, stepMax))
            {
                ...
                if (pcheck >= threshold)
                {
                    ...
                    GameObject.Instantiate(cloudPrefab,
                        ...
                        UnityEngine.Random.rotation) as GameObject;
